...
#         plot_gt(im, gt_boxes[:,:-1], im_path, mode = 'xyxya')
    ...
    def _load_annotation(self, index):
        ...
        filename = os.path.join(root_dir, 'gt_'+img_name[:-4]+'.txt')
...
class IC13Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_boxes = np.zeros((len(gt_inds), 6), dtype=np.float32)
        if self.augment :
            transform = Augment([ HSV_pos(0.5, 0.8, p=0.),
                                  HorizontalFlip(p=0.5),
                                  VerticalFlip(p=0.5),
                                  Affine(degree=0, translate=0.05, scale=0.1, p=0.5),  
                                  Noise(0.01, p=0.2),
                                  Blur(2.5, p=0.5),
                                ],box_mode = 'xyxyxyxy',)
            im, bboxes = transform(im, bboxes)
...
#         plot_gt(im, gt_boxes[:,:-1], im_path, mode = 'xyxya')
    ...
    def _load_annotation(self, index):
        root_dir, img_name = os.path.split(index)
        filename = os.path.join(root_dir.replace('images','labels'), 'gt_'+img_name[:-4]+'.txt')
        ...
        with open(filename,'r',encoding='utf-8-sig') as f:
            ...
            for obj in objects:
                if len(obj) != 0 :
                    label = 1
                    box = [ eval(x) for x in  obj.split(' ')[:4] ]
